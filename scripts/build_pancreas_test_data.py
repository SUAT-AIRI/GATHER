...
INPUT_H5AD = SCRIPT_DIR / "../data/TS_Pancreas.h5ad"
OUTPUT_DIR = SCRIPT_DIR / "../output/experiments/test_data"
OUTPUT_JSONL = OUTPUT_DIR / "pancreas_test.jsonl"
...
# 排除非标准 Cell Ontology 名称的细胞类型
EXCLUDED_CELL_TYPES = [
    "pancreatic beta cell",   # KG中为 type B pancreatic cell
    "pancreatic alpha cell",  # KG中为 pancreatic A cell
    "pancreatic delta cell",  # KG中为 pancreatic D cell
    "nk cell",                # KG中为 natural killer cell
]

# 大小写修正映射（数据集 → 标准 Cell Ontology）
CELL_TYPE_MAPPING = {
...
EXCLUDE_PATTERNS = [
    ...
    r'-AS\d*$',
    r'-DT$',
    # 额外排除 lncRNA / snoRNA / miRNA 等非编码基因
    r'^RNU\d',
    r'^RN7SL',
    r'^RN7SK',
    r'^MIR\d',
...
def compute_idf_weights(adata) -> dict:
    """使用 raw counts 计算 IDF 权重（避免 double-log 问题）。"""
    X = adata.raw.X
    n_cells = adata.n_obs
...
def stratified_sample(adata, test_ratio, excluded_types, seed):
    ...
    for ct_raw in unique_types:
        if ct_raw in excluded_types:
            print(f"  x {ct_raw}: excluded (non-standard name)")
            continue
...
def main():
    print("=" * 70)
    print("Build Pancreas Cell Test Dataset (TF-IDF Ranking)")
    print("=" * 70)
